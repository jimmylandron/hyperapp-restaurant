...
export default function Reviews({state, actions}) {
  ...
  return (
    ...
    <div class="col-md-8" side-img>
      <img src="https://s-i.huffpost.com/gen/1282658/images/o-FEMALE-CHEF-facebook.jpg" alt=""/>
    </div>
    <div class="col-md-4">
        {currentReview()}
      <div class="arrows" >
        <i onclick={leftClickedBTN} class={`fa fa-arrow-left ${(state.reviewStatus.currentReview) > 0 ? 'ready' : ''} `} aria-hidden="true"></i>
        <i onclick={rightClickedBTN} class={`fa fa-arrow-right ${(state.reviewStatus.currentReview) == (state.reviewsData.length - 1) ? '' : ' ready'} `} aria-hidden="true"></i>
      </div>
    </div>
    </div>
    </div>
